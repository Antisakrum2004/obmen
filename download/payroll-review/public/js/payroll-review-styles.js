/* payroll-review-styles.js v8.1.0 - Dark fintech dashboard */
...
var PR_CSS = [
  ...
  '.pr-kpi-hours{font-family:var(--mono);font-size:22px;font-weight:700;color:var(--text);line-height:1;letter-spacing:-.02em}',
  '.pr-compact .pr-kpi-hours{font-size:18px}',
  ...
  '.pr-kpi-money{font-family:var(--mono);font-size:22px;font-weight:700;color:var(--green);line-height:1}',
  '.pr-compact .pr-kpi-money{font-size:18px}',
  ...
  '.pr-card-breakdown{padding:8px 16px;background:rgba(0,0,0,.06);border-top:1px solid var(--border);font-family:var(--mono);font-size:10px;color:var(--text3);line-height:1.5}',
  '.pr-bd-item{white-space:nowrap}',
  '.pr-bd-sep{color:var(--text3);margin:0 1px}',
  '.pr-bd-green{color:var(--green);font-weight:600}',
  '.pr-bd-red{color:var(--red);font-weight:600}',
  ...
  '.pr-modal-body{padding:20px;overflow-y:auto;flex:1;scrollbar-width:none;-ms-overflow-style:none}',
  '.pr-modal-body::-webkit-scrollbar{display:none}',
  ...
  /* v8.1: Admin compact card layout */
  '.pr-admin-card{background:#1e2130;border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px;transition:border-color .3s,box-shadow .3s}',
  '.pr-admin-card.saved{border-color:var(--green);box-shadow:0 0 8px rgba(34,212,126,.2)}',
  '.pr-admin-card.flash{animation:prFlashGreen .6s ease}',
  '@keyframes prFlashGreen{0%{box-shadow:0 0 0 rgba(34,212,126,0)}50%{box-shadow:0 0 16px rgba(34,212,126,.4)}100%{box-shadow:0 0 0 rgba(34,212,126,0)}}',
  '.pr-admin-card-top{display:flex;align-items:center;gap:8px;margin-bottom:10px}',
  '.pr-admin-card-name{font-weight:600;font-size:13px;color:var(--text);flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
  '.pr-admin-card-role{font-family:var(--mono);font-size:9px;color:var(--text3);padding:2px 6px;border-radius:4px;background:rgba(79,139,255,.08);border:1px solid rgba(79,139,255,.15);text-transform:uppercase;letter-spacing:.04em}',
  '.pr-admin-chip{font-family:var(--mono);font-size:9px;font-weight:600;padding:3px 8px;border-radius:6px;cursor:pointer;background:rgba(124,108,255,.12);color:#7c6cff;border:1px solid rgba(124,108,255,.25);transition:all .15s;white-space:nowrap}',
  '.pr-admin-chip:hover{background:rgba(124,108,255,.2);border-color:rgba(124,108,255,.4)}',
  '.pr-admin-fields{display:flex;gap:10px;margin-bottom:10px;flex-wrap:wrap}',
  '.pr-admin-field{display:flex;flex-direction:column;gap:2px;flex:1;min-width:120px}',
  '.pr-admin-field label{font-family:var(--mono);font-size:8px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em}',
  '.pr-admin-field .pr-admin-input{width:100%}',
  '.pr-admin-fines{margin-bottom:8px}',
  '.pr-admin-fines-title{font-family:var(--mono);font-size:9px;color:var(--red);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;display:flex;align-items:center;gap:6px}',
  '.pr-admin-fine-row{display:flex;align-items:center;gap:6px;margin-bottom:4px}',
  '.pr-admin-fine-row .pr-admin-input{flex:1}',
  '.pr-admin-fine-row .pr-admin-input.fine-amount{max-width:80px;color:var(--red)}',
  '.pr-admin-fine-remove{background:rgba(255,79,106,.1);border:1px solid rgba(255,79,106,.3);color:var(--red);cursor:pointer;font-size:11px;padding:3px 7px;border-radius:6px;transition:all .15s;line-height:1}',
  '.pr-admin-fine-remove:hover{background:rgba(255,79,106,.2);border-color:var(--red)}',
  '.pr-admin-fine-add{font-family:var(--mono);font-size:10px;padding:4px 10px;border-radius:6px;cursor:pointer;border:1px dashed rgba(255,79,106,.3);background:transparent;color:var(--red);transition:all .15s}',
  '.pr-admin-fine-add:hover{background:rgba(255,79,106,.06);border-color:rgba(255,79,106,.5)}',
  '.pr-admin-footer{position:sticky;bottom:0;z-index:60;background:rgba(21,28,44,.96);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:1px solid var(--border);border-radius:10px;padding:10px 16px;margin-top:12px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 -2px 16px rgba(0,0,0,.3)}',
  '.pr-admin-footer-counter{font-family:var(--mono);font-size:11px;color:var(--text3)}',
  '.pr-admin-footer-counter span{color:var(--yellow);font-weight:700}',
  '.pr-admin-save-btn{font-family:var(--mono);font-size:12px;font-weight:700;padding:8px 20px;border-radius:8px;cursor:pointer;border:none;background:linear-gradient(135deg,#7c6cff,#5a4fd4);color:#fff;letter-spacing:.04em;text-transform:uppercase;transition:all .2s;box-shadow:0 2px 8px rgba(124,108,255,.3)}',
  '.pr-admin-save-btn:hover{transform:translateY(-1px);box-shadow:0 4px 16px rgba(124,108,255,.4)}',
  '.pr-admin-save-btn.flash{background:linear-gradient(135deg,#22d47e,#1a9e5c);box-shadow:0 4px 16px rgba(34,212,126,.4)}',
  /* v8.1: Sub-modal (Settings overlay) */
  '.pr-sub-modal-overlay{position:fixed;inset:0;z-index:1100;background:rgba(0,0,0,.5);backdrop-filter:blur(2px);display:flex;align-items:center;justify-content:center;padding:20px}',
  '.pr-sub-modal{background:var(--bg1);border:1px solid var(--border2);border-radius:var(--radius);width:100%;max-width:460px;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,.5)}',
  '.pr-sub-modal-header{display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--border)}',
  '.pr-sub-modal-title{font-family:var(--mono);font-size:13px;font-weight:700;color:var(--text)}',
  '.pr-sub-modal-body{padding:16px 18px;overflow-y:auto;flex:1}',
  '.pr-sub-modal-field{margin-bottom:10px}',
  '.pr-sub-modal-field label{display:block;font-family:var(--mono);font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px}',
  '.pr-sub-modal-field .pr-admin-input{width:100%}',
  '.pr-sub-modal-field select.pr-admin-input{cursor:pointer}',
  '.pr-sub-modal-footer{padding:12px 18px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:8px}',
  /* v8.2: Slider components (shared: table + admin) */
  '.pr-tl-presets{display:flex;gap:2px;align-items:center}',
  '.pr-tl-preset{font-family:var(--mono);font-size:8px;font-weight:700;padding:2px 6px;border-radius:3px;cursor:pointer;border:1px solid var(--border);background:var(--bg2);color:var(--text3);transition:all .15s;user-select:none}',
  '.pr-tl-preset:hover{border-color:var(--accent);color:var(--text2)}',
  '.pr-tl-preset.active{background:var(--accent);color:#fff;border-color:var(--accent)}',
  '.pr-tl-slider{width:60px;height:4px;-webkit-appearance:none;appearance:none;background:var(--bg);border-radius:2px;outline:none;cursor:pointer}',
  '.pr-tl-slider::-webkit-slider-thumb{-webkit-appearance:none;width:12px;height:12px;border-radius:50%;background:var(--accent);cursor:pointer;border:2px solid var(--bg2)}',
  '.pr-tl-slider::-moz-range-thumb{width:12px;height:12px;border-radius:50%;background:var(--accent);cursor:pointer;border:2px solid var(--bg2)}',
  '.pr-tl-slider:disabled{opacity:.4;cursor:not-allowed}',
  /* v8.2: Table slider cell */
  '.pr-table-slider-cell{min-width:180px}',
  '.pr-table-slider{display:flex;align-items:center;gap:3px;flex-wrap:nowrap}',
  '.pr-table-slider .pr-tl-preset{font-size:8px;padding:1px 5px}',
  '.pr-table-slider .pr-tl-slider{width:50px}',
  '.pr-table-slider-val{font-family:var(--mono);font-size:10px;font-weight:700;color:var(--accent);min-width:28px;text-align:right}',
  /* v8.4: Admin inline field row */
  '.pr-admin-row{display:flex;align-items:center;gap:6px;margin-bottom:6px}',
  '.pr-admin-row-label{font-family:var(--mono);font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;min-width:80px;white-space:nowrap}',
  '.pr-admin-row-input{background:var(--bg);border:1px solid var(--border);border-radius:6px;color:var(--text);font-family:var(--mono);font-size:12px;padding:5px 10px;outline:none;flex:1;min-width:60px;max-width:160px;transition:border-color .15s}',
  '.pr-admin-row-input:focus{border-color:var(--accent)}',
  /* hide number spinners */
  '.pr-admin-row-input::-webkit-outer-spin-button,.pr-admin-row-input::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}',
  '.pr-admin-row-input[type=number]{-moz-appearance:textfield}',
  '.pr-admin-row-unit{font-family:var(--mono);font-size:9px;color:var(--text3);white-space:nowrap}',
  '.pr-admin-confirm{background:none;border:1px solid var(--border);border-radius:6px;color:var(--text3);font-size:14px;cursor:pointer;padding:3px 7px;transition:all .15s;line-height:1}',
  '.pr-admin-confirm:hover{border-color:var(--green);color:var(--green);background:rgba(34,212,126,.08)}',
  '.pr-admin-confirm.saved{border-color:var(--green);color:var(--green);background:rgba(34,212,126,.15)}'
].join('');
