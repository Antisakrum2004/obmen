...
## v11.0 — МИКРОСЕРВИСНОЕ РАЗБИЕНИЕ МОНОЛИТА

Дата: 2026-05-28
Статус: **В РАБОТЕ**

### БЭКАП
- Каталог: `/home/z/my-project/backup/pre-microservice-split-2026-05-28/`
- Файлы: 15 (14 JS + PROJECT_BRAIN.md)
- Откат: `bash /home/z/my-project/backup/ROLLBACK.sh`
- Контрольная сумма монолита: `2275e9706302b24e867609407c33590bf23eb510dc8df263dbd2e8d34eba75d8`

### Статистика кода ДО разбиения

| Категория | Файлы | Строк | Доля |
|-----------|-------|------:|-----:|
| Монолит (UI) | tab-payroll-review.js + стили | 2 719 | 34% |
| Legacy (дубли) | calc, storage, export (корневые) | 502 | 6% |
| Infra | core.js, data-loader.js | 1 043 | 13% |
| Payroll Engine | 7 файлов в payroll/ | 3 632 | 46% |
| **ИТОГО** | **14 файлов** | **7 896** | 100% |

### Монолит — карта функций (2 397 строк)

| Сервис | Функций | Строк | Владеет state | Зависимости |
|--------|---------|------:|---------------|-------------|
| Orchestrator | 25 | ~750 | rows, projection, totals, data, loading | ВСЕ сервисы |
| AdminService | 8 | ~325 | modalOpen, adminTab, adminChangedDevs | Orchestrator (rebuild) |
| ReviewService | 12 | ~400 | dirty, sortField, expandedTaskEdit | Orchestrator (rebuild) |
| CardsService | 18 | ~550 | filters, expandedCards | Timeline, Orchestrator |
| TimelineService | 4 | ~120 | _taskDateCache | — (самый изолированный) |
| ExportService | 2 | ~40 | — | ReviewService (_prSaveAll) |
| HeaderService | 4 | ~100 | viewMode, densityMode, roleMode | Orchestrator (_prLoadData) |

### Ключевая проблема: 10x дублированный rebuild-каскад

10 функций содержат идентичный блок:
```
_pr._perf.projectionRebuilds++;
invalidateProjectionCache();
_pr.projection = buildMonthlyProjectionCached(_pr.rows);
_pr.totals = buildPeriodTotalsCached(_pr.rows);
_prEnsureAllDevsInProjection();
_prScheduleRender();
```

Дублировано в: _prPresetHours, _prPresetHoursTable, _prSliderBillable, _prSliderPayroll,
_prSliderBillableTable, _prSliderPayrollTable, _prOnEdit, _prCycleStatus,
_prApproveAll, _prSaveAdmin

**Решение**: Единый `_prRebuildAndRender()` в Orchestrator.

### План разбиения — 10 шагов

```
Шаг 1:  pr-bus.js (EventBus)               +40 строк  — НЕ ломает ничего
Шаг 2:  _prRebuildAndRender()               рефактор   — убирает 10x копипаст
Шаг 3:  AdminService (pr-admin.js)          ~325 строк — решает скролл-баг
Шаг 4:  TimelineService (pr-timeline.js)    ~120 строк — самый изолированный
Шаг 5:  ExportService (pr-export-svc.js)    ~40 строк  — тривиально
Шаг 6:  HeaderService (pr-header.js)        ~100 строк
Шаг 7:  ReviewService (pr-review.js)        ~400 строк
Шаг 8:  CardsService (pr-cards.js)          ~550 строк — зависит от Timeline
Шаг 9:  Orchestrator (остаток)              тонкий координатор
Шаг 10: Удалить legacy-файлы                −502 строк — payroll/ заменяет
```

### Event Bus — события (10 штук)

| Событие | Кто шлёт | Кто слушает | Payload |
|---------|----------|-------------|---------|
| `rows:mutated` | Review, Admin | Orchestrator → rebuild | {source} |
| `rates:updated` | Admin | Orchestrator → rebuild | {changedDevIds[]} |
| `review:status-changed` | Review | Orchestrator → rebuild | {rowIdx, old, new} |
| `period:changed` | Header | Orchestrator → reload | {year, month} |
| `filters:changed` | Cards | Orchestrator → render | {filters} |
| `card:toggled` | Cards | Cards → partial | {devId} |
| `task:edit-toggled` | Review | Timeline → render | {editKey} |
| `reviews:saved` | Review | Orchestrator → clear dirty | {periodKey} |
| `period:approved` | Review | Orchestrator → snapshot | {periodKey} |
| `admin:save-complete` | Admin | Orchestrator → rebuild | {} |

### Правило разбиения: «Каждый шаг — рабочее приложение»

После КАЖДОГО шага приложение должно работать без изменений в поведении.
Новые файлы подключаются через `<script>` в том же порядке.
Старые функции делегируют к новым (не удаляются сразу).
...
├── data-loader.js               — Pipeline загрузки данных (v7.2.0)
├── pr-bus.js                    — EventBus (НОВЫЙ)
├── pr-admin.js                  — AdminService (НОВЫЙ, из монолита)
├── pr-timeline.js               — TimelineService (НОВЫЙ, из монолита)
├── pr-export-svc.js             — ExportService (НОВЫЙ, из монолита)
├── pr-header.js                 — HeaderService (НОВЫЙ, из монолита)
├── pr-review.js                 — ReviewService (НОВЫЙ, из монолита)
├── pr-cards.js                  — CardsService (НОВЫЙ, из монолита)
├── tab-payroll-review.js        — Orchestrator (остаток монолита, ~750 строк)
├── payroll-review-styles.js     — CSS стили
└── payroll/
    ├── payroll-domain.js        — Domain types, constants
    ├── payroll-storage.js       — Persistence layer
    ├── payroll-projection.js    — Прогнозы, агрегация
    ├── payroll-export.js        — Export DTO + CSV serialization
    ├── payroll-review-engine.js — Движок ревью
    ├── payroll-normalizer.js    — Нормализация elapsed
    └── payroll-cache.js         — Smart Cache Layer

УДАЛИТЬ после шага 10:
├── payroll-review-calc.js       — Legacy, заменён payroll/
├── payroll-review-storage.js    — Legacy, заменён payroll/
├── payroll-review-export.js     — Legacy, заменён payroll/
```

### Прогресс

- [x] Шаг 1: pr-bus.js → PayrollEvents УЖЕ СУЩЕСТВУЕТ в payroll-domain.js
- [x] Шаг 2: _prRebuildAndRender() — 12 каскадов заменены на единый вызов
- [x] Шаг 3: AdminService — pr-admin.js (467 строк), решает скролл-баг
- [ ] Шаг 4: TimelineService
- [ ] Шаг 5: ExportService
- [ ] Шаг 6: HeaderService
- [ ] Шаг 7: ReviewService
- [ ] Шаг 8: CardsService
- [ ] Шаг 9: Orchestrator cleanup
- [ ] Шаг 10: Delete legacy
