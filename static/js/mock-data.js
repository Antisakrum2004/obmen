/* ═══════════════════════════════════════════════════════════════
   ...
   v8.0.0 — TRUE ELAPSED-FIRST: day-by-day pipeline

   Pipeline:
   1. LOAD ELAPSED DAY-BY-DAY via task.elapseditem.getlist (array body format)
   2. FILTER by developer IDs
   3. EXTRACT UNIQUE TASK IDS from elapsed
   4. LOAD ONLY REFERENCED TASKS by ID (batches of 50)
   5. NORMALIZE + BUILD MODEL
   6. CACHE

   Source of truth for payroll = elapsed entries, NOT tasks.task.list
   Same method as reference-dashboard/js/core.js loadPeriodData()

   Critical constraints:
   - Day-by-day elapsed loading (same as reference-dashboard)
   - Body must be ARRAY format: [ORDER, {}, FILTER, SELECT]
   - Max 5 concurrent day requests (rate limit safety)
...
/* ═══════════════════════════════════════════════════════════════
   REAL DATA LOADER — TRUE ELAPSED-FIRST Pipeline v8.0.0

   NEW PIPELINE (same as reference-dashboard):
   1. Load developers (cached)
   2. Load elapsed DAY-BY-DAY via task.elapseditem.getlist with date filter
   3. Filter elapsed by our developer IDs
   4. Extract unique task IDs from elapsed
   5. Load ONLY referenced tasks by ID (batches of 50)
   6. Build metadata
   7. Load projects (cached)
   8. Cache result

   PERIOD BOUNDARIES: current month only
   MAX CONCURRENCY: 5 API calls per day batch
...
function PR_loadRealData(year, month, progressCb) {
  ...
  console.log('PR_loadRealData v8.0: загрузка за ' + fromStr + ' — ' + toStr + ' (day-by-day elapsed-first)');
...
function _prLoadRealDataFresh(year, month, fromStr, toStr, periodKey, cacheKey, progressCb) {
  ...
  return devPromise.then(function() {
    ...
    return tasksPromise.then(function(allTasks) {
      ...
      /* v8.0: НЕ фильтруем elapsed по EXCLUDE_GROUPS.
         Все elapsed записи сохраняются для подсчёта Факт часов.
         Исключение из оплаты обрабатывается через UNPAID_GROUPS в review engine.
         Elapsed уже отфильтрован по нашим developer IDs в _prLoadElapsedByDev. */
...
/* ─── Load elapsed DAY-BY-DAY (TRUE ELAPSED-FIRST pipeline) ───
   v8.0.0 — Complete rewrite based on reference-dashboard method.
   
   ROOT CAUSE of missing hours:
   v7.3.3 used CHANGED_DATE to find tasks first, then loaded elapsed for those tasks.
   This MISSED tasks that had elapsed in the period but weren't recently changed.
   Приходько lost 72h, Макаров 11h, Попов 10h due to this.
   
   NEW Pipeline (same as reference-dashboard/js/core.js loadPeriodData):
   1) Call task.elapseditem.getlist PER DAY with array body format
      Body: [ORDER, {}, {>=CREATED_DATE, <=CREATED_DATE}, SELECT]
      This loads ALL elapsed for the day regardless of task ownership
   2) Filter elapsed by our developer IDs
   3) Extract unique task IDs from elapsed
   4) Load task details by ID in batches of 50
   
   CRITICAL: Body must be ARRAY format, not object!
   CRITICAL: No TASK_ID required — just date range filter!
   Reference: reference-dashboard/js/core.js line 50 */
function _prLoadElapsedByDev(fromStr, toStr, progressCb) {
  var devIds = (typeof ACTIVE_DEV_IDS !== 'undefined') ? ACTIVE_DEV_IDS : DEV_IDS;
  var cb = progressCb || function() {};

  console.log('_prLoadElapsedByDev v8.0: TRUE ELAPSED-FIRST, period ' + fromStr + ' — ' + toStr);

  /* Step 1: Generate all dates in the period */
  var dates = [];
  ...
  cb('Загрузка elapsed', dates.length + ' дней напрямую из API...');

  /* Step 2: Load elapsed PER DAY using task.elapseditem.getlist
     Same method as reference-dashboard: array body format [ORDER, {}, DATE_FILTER, SELECT]
     This is the ONLY reliable way to get ALL elapsed entries. */
  var hook = HOOK ? HOOK.trim() : '';
  var devSet = {};
  devIds.forEach(function(id) { devSet[String(id)] = true; });

  /* Throttle: max 5 concurrent day requests to avoid rate limits */
  var CONCURRENT_DAYS = 5;
  var allElapsed = [];
  var dateIdx = 0;
  var activeCount = 0;
  var resolveAll;
  ...
  function loadNextDay() {
    while (activeCount < CONCURRENT_DAYS && dateIdx < dates.length) {
      var ds = dates[dateIdx];
      dateIdx++;
      activeCount++;
      ...
      fetch(u, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: b})
        ...
        .then(function(d) {
          activeCount--;
          if (d && !d.error && Array.isArray(d.result)) {
            /* Filter by our developer IDs immediately */
            d.result.forEach(function(e) {
              if (devSet[String(e.USER_ID)]) {
                allElapsed.push(e);
              }
            });
          } else if (d && d.error) {
            console.warn('_prLoadElapsedByDev: API error for day ' + ds + ':', d.error);
          }
          /* Progress callback */
          var pct = Math.round((dateIdx / dates.length) * 100);
          cb('Elapsed', dateIdx + '/' + dates.length + ' дней (' + allElapsed.length + ' записей) ' + pct + '%');
          loadNextDay();
        })
        .catch(function(e) {
          activeCount--;
          console.warn('_prLoadElapsedByDev: network error for day ' + ds, e);
          loadNextDay();
        });
    }

    if (dateIdx >= dates.length && activeCount === 0) {
      resolveAll();
    }
  }
  ...
  return new Promise(function(resolve) {
    ...
    if (!dates.length) {
      resolve({ elapsed: [], tasks: [] });
      return;
    }

    loadNextDay();
  }).then(function() {
    console.log('_prLoadElapsedByDev: получено ' + allElapsed.length + ' elapsed записей за ' + dates.length + ' дней');

    /* Safety limit */
    if (allElapsed.length > PR_MAX_ELAPSED) {
      console.warn('_prLoadElapsedByDev: SAFETY LIMIT — обрезаем elapsed с ' + allElapsed.length + ' до ' + PR_MAX_ELAPSED);
      allElapsed = allElapsed.slice(0, PR_MAX_ELAPSED);
    }
    ...
    allElapsed.forEach(function(e) {
      ...
      if (tid && !taskIdsSet[tid]) {
        taskIdsSet[tid] = true;
        taskIds.push(tid);
      }
    });

    cb('Извлечение ID', taskIds.length + ' уникальных задач из ' + allElapsed.length + ' elapsed');
    console.log('_prLoadElapsedByDev: ' + taskIds.length + ' уникальных задач');

    if (!taskIds.length) {
      return { elapsed: allElapsed, tasks: [] };
    }

    /* Safety limit */
    if (taskIds.length > PR_MAX_TASKS) {
      console.warn('_prLoadElapsedByDev: SAFETY LIMIT — обрезаем задачи с ' + taskIds.length + ' до ' + PR_MAX_TASKS);
      taskIds = taskIds.slice(0, PR_MAX_TASKS);
    }

    /* Step 4: Load task details by ID in batches of 50
       Same method as reference-dashboard: bxPost('tasks.task.list', {filter: {ID: batch}, select: [...]}) */
    cb('Загрузка задач', taskIds.length + ' по ID (батчами по 50)...');

    var allTasks = [];
    var batchIdx = 0;

    function loadNextBatch() {
      if (batchIdx >= taskIds.length) {
        return Promise.resolve(allTasks);
      }
      var batch = taskIds.slice(batchIdx, batchIdx + 50);
      batchIdx += 50;

      return bxPost('tasks.task.list', {
        filter: {ID: batch},
        select: ['ID', 'TITLE', 'GROUP_ID', 'STAGE_ID', 'CREATED_DATE', 'CLOSED_DATE', 'STATUS', 'RESPONSIBLE_ID', 'DEADLINE', 'DESCRIPTION']
      }, 30000).then(function(r) {
        var tasks = (r && r.result && r.result.tasks) || (r && r.result) || [];
        if (!Array.isArray(tasks)) tasks = [];
        allTasks = allTasks.concat(tasks);

        var pct = Math.round((Math.min(batchIdx, taskIds.length) / taskIds.length) * 100);
        cb('Задачи', Math.min(batchIdx, taskIds.length) + '/' + taskIds.length + ' (' + pct + '%)');

        return loadNextBatch();
      }).catch(function(e) {
        console.warn('_prLoadElapsedByDev: batch error at ' + batchIdx, e);
        return loadNextBatch();
      });
    }

    return loadNextBatch().then(function(allTasks) {
      cb('Задачи загружены', allTasks.length + ' задач из API');
      console.log('_prLoadElapsedByDev: загружено ' + allTasks.length + ' задач');
